...
app.get('/api/candidates/export', requireAuth, asyncRoute(async (req, res) => {
  if (!canManageUsers(req.user)) return res.status(403).json({ error: 'Only manager can export candidates.' });
...
app.get('/api/candidates/template', requireAuth, asyncRoute(async (req, res) => {
  if (!canManageUsers(req.user)) return res.status(403).json({ error: 'Only manager can download templates.' });
...
app.post('/api/candidates/import', requireAuth, upload.single('file'), asyncRoute(async (req, res) => {
  if (!canManageUsers(req.user)) return res.status(403).json({ error: 'Only manager can import candidates.' });
