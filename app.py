...
DB_PATH = DATA_DIR / "career_crox_fallback.db"
...
def is_pg():
    return DB_BACKEND == "postgres" and DATABASE_URL and psycopg is not None
...
def q_all(sql, params=()):
    cur = get_db().execute(sqlize(sql), params)
...
def execute(sql, params=()):
    db = get_db()
    cur = db.execute(sqlize(sql), params)
    db.commit()
    return cur
...
def execute_many(sql, rows):
    db = get_db()
    db.executemany(sqlize(sql), rows)
    db.commit()
...
def insert_and_get_id(sql, params=()):
    db = get_db()
    if is_pg():
        cur = db.execute(sqlize(sql + " returning id"), params)
        row = cur.fetchone()
        db.commit()
...
def run_schema_migrations(db):
    ...
    if "next_followup_at" not in profile_columns:
        db.execute("alter table profiles add column next_followup_at text")
    if "followup_notified_at" not in profile_columns:
        db.execute("alter table profiles add column followup_notified_at text")
    db.execute("create index if not exists idx_profiles_followup on profiles(next_followup_at)")
...
def dashboard():
    ...
    cards = {
        "profiles_today": q_one("select count(*) c from profiles " + where + (" and " if where else " where ") + "substr(created_at,1,10)=?", params + [today_str()])["c"],
        ...
        "interviews_today": q_one("select count(*) c from interviews " + recruiter_only_filter("")[0].replace("recruiter_code", "recruiter_code") + (" and " if recruiter_only_filter("")[0] else " where ") + "substr(interview_at,1,10)=?", recruiter_only_filter("")[1] + [today_str()])["c"],
...
def interviews():
    ...
    if is_recruiter():
        ...
        params.append(session["user_code"])
    if tab == "today":
        filters.append("substr(i.interview_at,1,10)=?")
        params.append(today_str())
    elif tab == "upcoming":
        filters.append("substr(i.interview_at,1,10)>?")
        params.append(today_str())
    elif tab == "due":
        filters.append("substr(i.interview_at,1,10)<? and i.status!='completed'")
        params.append(today_str())
