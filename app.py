...
def derive_seed_payload(xlsx_path):
    ...
    users = [
        ...
        {"user_id": "U002", "username": "aaryansh.manager", "password": "Manager@123", "full_name": "Aaryansh Patel", "designation": "Manager", "role": "manager", "recruiter_code": "MGR-001", "is_active": "1", "theme_name": "corporate-light", "updated_at": now_iso()},
        {"user_id": "U003", "username": "sakshi.tl", "password": "TL@12345", "full_name": "Sakshi Soni", "designation": "Team Lead", "role": "tl", "recruiter_code": "TL-001", "is_active": "1", "theme_name": "corporate-light", "updated_at": now_iso()},
        {"user_id": "U004", "username": "anjali.tl", "password": "TL@12345", "full_name": "Anjali Shrivastava", "designation": "Team Lead", "role": "tl", "recruiter_code": "TL-002", "is_active": "1", "theme_name": "corporate-light", "updated_at": now_iso()},
        {"user_id": "U005", "username": "arjun.rec", "password": "Rec@12345", "full_name": "Arjun Recruiter", "designation": "Recruiter", "role": "recruiter", "recruiter_code": "RC-201", "is_active": "1", "theme_name": "corporate-light", "updated_at": now_iso()},
        {"user_id": "U006", "username": "rabia.rec", "password": "Rec@12345", "full_name": "Rabia Mann", "designation": "Recruiter", "role": "recruiter", "recruiter_code": "RC-202", "is_active": "1", "theme_name": "corporate-light", "updated_at": now_iso()},
        {"user_id": "U007", "username": "tahseen.rec", "password": "Rec@12345", "full_name": "Tahseen Naaz", "designation": "Recruiter", "role": "recruiter", "recruiter_code": "RC-203", "is_active": "1", "theme_name": "corporate-light", "updated_at": now_iso()},
        {"user_id": "U008", "username": "pragya.rec", "password": "Rec@12345", "full_name": "Pragya Singh", "designation": "Recruiter", "role": "recruiter", "recruiter_code": "RC-204", "is_active": "1", "theme_name": "corporate-light", "updated_at": now_iso()},
    ]
    if not candidates:
        candidates = [
...
def add_note(candidate_code):
    ...
    if not body:
        flash("A note cannot be empty.", "danger")
...
def meeting_room_urls():
    return {
        "room_a": {
            "key": "room_a",
            "label": "Room A",
            "url": (os.environ.get("MEETING_ROOM_A_URL", "") or "").strip(),
            "hint": "First Google Meet room. Keep up to 10 employees here for cleaner shared-screen view.",
        },
        "room_b": {
            "key": "room_b",
            "label": "Room B",
            "url": (os.environ.get("MEETING_ROOM_B_URL", "") or "").strip(),
            "hint": "Second Google Meet room. Use this when team size goes beyond 10 screens.",
        },
    }


def build_meeting_people():
    try:
    ...
    presence_map = {}
    try:
        presence_map = {str(r.get("user_id") or ""): dict(r) for r in get_backend().list_rows("presence")}
    except Exception:
        presence_map = {}
    role_rank = {"manager": 0, "tl": 1, "recruiter": 2}
    users = [dict(u) for u in list_users() if to_boolish(u.get("is_active", "1"))]
    users.sort(key=lambda u: (
        role_rank.get(normalize_role(u.get("role"), u.get("designation")), 9),
        (u.get("full_name") or u.get("username") or "").lower(),
    ))
    people = []
    for idx, emp in enumerate(users):
        role = normalize_role(emp.get("role"), emp.get("designation"))
        presence = presence_map.get(str(emp.get("user_id") or ""), {})
        room_key = "room_a" if idx % 2 == 0 else "room_b"
        if role == "manager":
            room_key = "room_a"
        name = (emp.get("full_name") or emp.get("username") or "Employee").strip()
        parts = [chunk[0].upper() for chunk in name.split()[:2] if chunk]
        initials = "".join(parts) or "?"
        locked = to_boolish(presence.get("locked"))
        screen_sharing = to_boolish(presence.get("screen_sharing"))
        joined = to_boolish(presence.get("meeting_joined"))
        if locked:
            state_label = "Locked"
            state_class = "locked"
        elif screen_sharing:
            ...
            state_label = "Ready"
            state_class = "ready"
        role_label = "Manager" if role == "manager" else ("Team Lead" if role == "tl" else "Recruiter")
        code = (emp.get("recruiter_code") or emp.get("username") or "").strip()
        meta_bits = [role_label]
        if code:
            meta_bits.append(code)
        last_seen = display_ts(presence.get("last_seen_at"), "No recent ping")
        people.append({
            "user_id": emp.get("user_id") or "",
            "name": name,
            "username": emp.get("username") or "",
            "designation": emp.get("designation") or role_label,
            "role": role,
            "role_label": role_label,
            "code": code,
            "room_key": room_key,
            "room_label": "Room A" if room_key == "room_a" else "Room B",
...
def module_page(slug):
    module = dict(MODULE_SUMMARIES.get(slug) or {})
    if not module:
        abort(404)
    user = current_user()
    if slug in {'wallet-rewards','payout-tracker'} and normalize_role(user.get('role'), user.get('designation')) != 'manager':
        flash('This section is available to managers only.', 'warning')
        return redirect(url_for('dashboard'))
    dialer_candidates = visible_candidates_rows(user) if slug == "dialer" else []
    meeting_feed = []
    meeting_people = []
    meeting_rooms = {}
    learning_videos = []
    if slug == 'meeting-room':
        meeting_rooms = meeting_room_urls()
        meeting_people = build_meeting_people()
        sharing_count = sum(1 for p in meeting_people if p.get("screen_sharing"))
        ready_count = sum(1 for p in meeting_people if not p.get("locked"))
        configured_links = sum(1 for room in meeting_rooms.values() if room.get("url"))
        module["cards"] = [
            ("Employees", str(len(meeting_people))),
            ("Sharing Now", str(sharing_count)),
            ("Ready to Join", str(ready_count)),
            ("Room Links", str(configured_links or 2)),
        ]
        meeting_feed = [
            {
                "name": person.get("name", "Employee"),
                "time": person.get("last_seen_label", "No recent ping"),
                "state": person.get("state_label", "Ready"),
            }
            for person in meeting_people[:8]
        ]
    ...
    return render_template(
        ...
        module=module,
        slug=slug,
        dialer_candidates=dialer_candidates,
...
def attendance_request_unlock():
    ...
    flash("The unlock request was sent to the Team Lead and Manager.", "success")
    return redirect(url_for("attendance_breaks"))
...
def aaria_parse_instruction(command_text, serial_hint=""):
    ...
    action = None; payload = {}
    if not raw:
        return {"ok": False, "message": "Instruction not clear. The command is empty.", "action": "none"}
    ...
    if action != "theme" and not candidate:
        return {"ok": False, "message": "Candidate not found. Enter the serial, phone digits, or candidate ID again.", "action": action}
    msg = ""
    ...
    elif action == "interview_date": msg = f"{candidate.get('full_name')} interview is ready to be scheduled for {payload['scheduled_at']}."
...
def aaria_execute_instruction(command_text, serial_hint="", preview=False):
    ...
    if action == "theme":
        ...
        phone = normalize_phone(candidate.get("phone")); response["message"] = f"Opening the dialer for {candidate.get('full_name')}."; response["action_link"] = f"tel:+91{phone}"; response["action_type"] = "call"; response["avatar_state"] = "call"; log_activity(user, "aaria_call", candidate.get("candidate_id"), {"phone": phone})
...
def schedule_report():
    ...
    get_backend().insert("scheduled_reports", row); flash("The report schedule was saved. A file will be generated automatically at the next run time, and a notification will be sent.", "success"); return redirect(url_for("reports_page", report_type=row["report_type"]))
...
def aaria_parse_instruction_v2(command_text, serial_hint=""):
    ...
    if any(k in text for k in ["theme", "color theme", "colour theme"]):
        ...
        if not msg and ("waiting" in text or "wait" in text):
            msg = "Your profile is currently in the waiting stage. You will be informed as soon as there is a shortlist update."
        if candidate:
    ...
    if action in {"call", "whatsapp", "candidate_status", "interview_date", "add_note", "phone_update"} and not candidate:
        return {"ok": False, "message": "Candidate not found. Use the candidate name, candidate ID, serial, or phone digits.", "action": action}
    if action == "task_update" and not task:
        return {"ok": False, "message": "Task not found. Use a task ID such as T001.", "action": action}
    msg = "Ready."
    if action == "candidate_status":
        msg = f"{candidate.get('full_name')} ko {payload['status']} mark karne ke liye ready."
    elif action == "interview_date":
        msg = f"{candidate.get('full_name')} interview is ready to be scheduled for {payload['scheduled_at']}."
    elif action == "add_note":
...
def aaria_execute_single_v2(command_text, serial_hint="", preview=False):
    ...
    if action == "theme":
        ...
        response["message"] = f"Opening the dialer for {candidate.get('full_name')}."
        ...
        response["message"] = f"{candidate.get('full_name')} was marked as {payload['status']}."
        response["avatar_state"] = "success"
    elif action == "interview_date":
        existing = next((dict(i) for i in get_backend().list_rows("interviews") if i.get("candidate_id") == candidate.get("candidate_id")), None)
        jd_match = next((j for j in get_backend().list_rows("jd_master") if (j.get("company") or "").strip().lower() == ((candidate.get("process") or "").split(',')[0].strip().lower())), None)
        ...
        response["message"] = f"A note was added for {candidate.get('full_name')}."
        response["avatar_state"] = "success"
        ...
        response["message"] = f"{candidate.get('full_name')}  phone number was updated."
        response["avatar_state"] = "success"
        ...
        response["message"] = f"Task {task_id} was created successfully."
        response["avatar_state"] = "success"
        ...
        response["message"] = f"Task {task.get('task_id')} was updated to {payload.get('status')}."
        response["avatar_state"] = "success"
...
def aaria_execute_batch(command_text, serial_hint="", preview=False):
    ...
    if not commands:
        return {"ok": False, "message": "Instruction not clear. The command is empty.", "batch_results": []}
    results = []
    for cmd in commands:
...
def lock_reason_message(reason):
    ...
    if "Late return from break" in reason:
        return "The CRM was auto-locked 90 seconds after the break end time. Unlock approval is required from a Team Lead or Manager."
...
def create_unlock_request_if_missing(user, reason):
    ...
    notify_users([u.get("user_id") for u in approver_users_for(user)], "Unlock request", f"{user.get('full_name')} requires unlock approval.", "attendance", {"user_id": user.get("user_id"), "reason": reason})
    return row
...
def attendance_request_unlock_v3():
    ...
    create_unlock_request_if_missing(user, reason)
    get_backend().update_where("presence", {"user_id": user.get("user_id")}, {"locked": "1", "last_seen_at": now_iso()})
    flash("The unlock request was sent to the approval team.", "success")
    return redirect(url_for("attendance_breaks"))
...
def chat_create_group():
    ...
    if not title or len(members) < 2:
        flash("A group title and at least 2 members are required.", "danger")
...
def aaria_parse_instruction_v3(command_text, serial_hint=''):
    ...
    text = raw.lower()
    if not raw:
        return {'ok': False, 'message': 'The command is empty.', 'action': 'none', 'fix_suggestion': aaria_fix_suggestion(raw)}
    user = current_user()
    ...
    if not action:
        return {'ok': False, 'message': 'The command is not clear.', 'action': 'none', 'candidate': candidate, 'fix_suggestion': aaria_fix_suggestion(raw)}
...
def aaria_execute_batch_v3(command_text, serial_hint='', preview=False):
    ...
    if not commands:
        return {'ok': False, 'message': 'The command is empty.', 'batch_results': [], 'fix_suggestion': aaria_fix_suggestion(command_text)}
    results = []
    for cmd in commands:
...
def recruiter_performance_rows(selected_username=''):
    ...
    for user in users:
        ...
        row = {
            ...
            'idle_state': 'No movement detected for the past 15 minutes' if idle_minutes >= 15 and idle_minutes < 999 else 'Active',
            'idle_alert': idle_minutes >= 15 and idle_minutes < 999,
        }
        rows.append(row)
...
def lock_reason_message(reason):
    reason = (reason or "").strip()
    if "Late return from break" in reason:
        return "The recruiter CRM was auto-locked because the break ended and no return activity was detected. Unlock approval is required from a Team Lead or Manager."
    if "No call dialed" in reason:
        return "The recruiter CRM was auto-locked because no call was dialed for 15 minutes. Unlock approval is required from a Team Lead or Manager."
    if "Inactivity" in reason:
        return "The recruiter CRM was auto-locked because no activity was detected for 5 minutes. Unlock approval is required from a Team Lead or Manager."
    return reason or "CRM locked. Unlock approval required."
...
def maybe_emit_due_followup_notifications(user):
    ...
    for cand in visible_candidates_rows(user):
        ...
        if _notification_fingerprint_exists(user.get("user_id"), fingerprint):
            continue
        notify_users([user.get("user_id")], "Follow-up reminder", f"The follow-up for {cand.get('full_name')} is now due.", "followup", {"candidate_id": cand.get("candidate_id"), "fingerprint": fingerprint, "follow_up_at": cand.get("follow_up_at")})
...
def update_candidate_v8(candidate_code):
    ...
    try:
        ...
        flash("The candidate could not be saved because one or more fields are not supported by the current database schema. Core data is safe. Please open the profile again and save once more.", "danger")
    return redirect(url_for("candidate_detail", candidate_code=candidate_code))
...
def attendance_request_unlock_v4():
    ...
    if normalize_role(user.get("role")) in UNLOCK_EXEMPT_ROLES:
        flash("Unlock approval does not apply to Manager or Team Lead accounts.", "info")
        return redirect(url_for("attendance_breaks"))
    reason = request.form.get("reason", "Unlock requested").strip() or "Unlock requested"
    create_unlock_request_if_missing(user, reason)
    get_backend().update_where("presence", {"user_id": user.get("user_id")}, {"locked": "1", "last_seen_at": now_iso()})
...
def create_task_v3():
    ...
    if normalize_role(creator.get("role")) not in PRIVILEGED_ROLES:
        flash("Task creation is available only for Team Lead and Manager accounts.", "danger")
        return redirect(url_for("tasks"))
    target = find_user_by_hint(request.form.get("assigned_to_username", ""))
    if not target:
        flash("The assigned username or name was not found.", "danger")
        return redirect(url_for("tasks"))
    row = {
        "task_id": next_prefixed_id("tasks", "task_id", "T"),
        "title": request.form.get("title", "").strip(),
    ...
    if not row["title"]:
        flash("A task title is required.", "danger")
        return redirect(url_for("tasks"))
    try:
        get_backend().insert("tasks", row)
        ...
        flash("The task was added successfully.", "success")
    except Exception:
        app.logger.exception("Task create failed for %s", target.get("username"))
        flash("The task could not be saved because the server rejected one or more fields. Please try again.", "danger")
    return redirect(url_for("tasks"))
...
def api_dialer_call_end():
    ...
    return jsonify({'ok': True, 'message': f"The call for {candidate.get('full_name')} was saved with status {outcome}. Duration {_dialer_duration_label(duration_seconds)}."})
...
def api_dialer_note():
    ...
    if not note:
        return jsonify({'ok': False, 'message': 'A note cannot be empty.'}), 400
    ...
    try:
        ...
        return jsonify({'ok': False, 'message': 'The note could not be saved on the server.'}), 500
    return jsonify({'ok': True, 'message': f"The note for {candidate.get('full_name')} was saved successfully."})
...
# === speed + Supabase compatibility helpers ===
_PROCESS_TABLE_CACHE = {}
_PROCESS_CACHE_TTL_DEFAULT = 12
_PROCESS_CACHE_TTL = {
    "users": 30,
    "jd_master": 30,
    "settings": 30,
...
def _table_cache_ttl(table_name):
    return _PROCESS_CACHE_TTL.get(table_name, _PROCESS_CACHE_TTL_DEFAULT)


def _process_cache_get(table_name):
    item = _PROCESS_TABLE_CACHE.get(table_name)
    if not item:
        return None
    if item.get("expires_at", 0) < time.time():
        _PROCESS_TABLE_CACHE.pop(table_name, None)
        return None
    return [dict(r) for r in item.get("rows", [])]


def _process_cache_set(table_name, rows):
    _PROCESS_TABLE_CACHE[table_name] = {
        "expires_at": time.time() + _table_cache_ttl(table_name),
        "rows": [dict(r) for r in rows],
    }


def invalidate_process_table_cache(*tables):
    if not tables:
        return
    for table in tables:
        _PROCESS_TABLE_CACHE.pop(table, None)


def _supabase_missing_column_name(exc, table_name=""):
    msg = str(exc or "")
    patterns = [
        r"Could not find the ['\"]?([A-Za-z0-9_]+)['\"]? column",
        r"column ['\"]?([A-Za-z0-9_]+)['\"]? of relation",
        r"schema cache.*?['\"]([A-Za-z0-9_]+)['\"]",
    ...
    for pattern in patterns:
        ...
        if m:
            return m.group(1)
    if table_name:
        for column in sorted(TABLE_COLUMNS.get(table_name, set()), key=len, reverse=True):
            if column and column in msg:
                return column
    return ""


def _strip_unsupported_supabase_columns(table_name, payload, exc):
    missing = _supabase_missing_column_name(exc, table_name)
    if not missing:
...
def _supabase_retry_insert(raw_insert, table_name, row_or_rows):
    payload = row_or_rows
    for _ in range(12):
        try:
            return raw_insert(table_name, payload)
        except Exception as exc:
            payload, changed = _strip_unsupported_supabase_columns(table_name, payload, exc)
            if not changed:
                raise
    return raw_insert(table_name, payload)


def _supabase_retry_update(raw_update_where, table_name, filters, values):
    payload = dict(values)
    for _ in range(12):
        try:
            return raw_update_where(table_name, filters, payload)
        except Exception as exc:
            payload, changed = _strip_unsupported_supabase_columns(table_name, payload, exc)
            if not changed:
                raise
...
def _build_runtime_safe_backend(backend):
    ...
    def runtime_safe_list_rows(table_name, *args, **kwargs):
        ...
        if use_cache:
            ...
            if table_name in cache:
                return [dict(r) for r in cache[table_name]]
            process_rows = _process_cache_get(table_name)
            if process_rows is not None:
                cache[table_name] = [dict(r) for r in process_rows]
                return [dict(r) for r in process_rows]
        try:
            rows = raw_list_rows(table_name, *args, **kwargs)
            rows = [dict(r) for r in rows]
            if use_cache:
                cache[table_name] = [dict(r) for r in rows]
                _process_cache_set(table_name, rows)
            return rows
        except Exception as exc:
    ...
    def runtime_safe_insert(table_name, row):
        try:
            ...
            invalidate_process_table_cache(table_name)
            if isinstance(backend, SupabaseBackend):
                return _supabase_retry_insert(raw_insert, table_name, row)
    ...
    def runtime_safe_bulk_insert(table_name, rows):
        try:
            ...
            invalidate_process_table_cache(table_name)
            if isinstance(backend, SupabaseBackend):
                return _supabase_retry_insert(raw_bulk_insert, table_name, rows)
    ...
    def runtime_safe_update_where(table_name, filters, values):
        try:
            ...
            invalidate_process_table_cache(table_name)
            if isinstance(backend, SupabaseBackend):
                return _supabase_retry_update(raw_update_where, table_name, filters, values)
...
def get_backend():
    global _SQLITE_FALLBACK_LOGGED, _BACKEND_SINGLETON, _BACKEND_SINGLETON_KIND
    backend = getattr(g, "backend", None)
    if backend is not None:
        return _build_runtime_safe_backend(backend)

    if _BACKEND_SINGLETON is not None:
        g.backend = _build_runtime_safe_backend(_BACKEND_SINGLETON)
        return g.backend
    ...
    if USE_SUPABASE:
        try:
            ...
            try:
                ...
                raise RuntimeError(str(probe_exc))
            _BACKEND_SINGLETON = backend
            _BACKEND_SINGLETON_KIND = "supabase"
    ...
    backend = SQLiteBackend(DB_PATH, SEED_FILE)
    _BACKEND_SINGLETON = backend
    _BACKEND_SINGLETON_KIND = "sqlite"
    g.backend = _build_runtime_safe_backend(backend)
    return g.backend
        ...
        return render_template_string(
            ...
            <body style="font-family:Arial,sans-serif;background:#f6f8fb;padding:32px;">
                <div style="max-width:720px;margin:0 auto;background:#fff;border-radius:14px;padding:24px;box-shadow:0 10px 30px rgba(0,0,0,.08);">
                    ...
                    <p>The app hit an error. Crashing silently is not a feature.</p>
                    <p>Check <strong>flask_error.log</strong> in the data folder. If you are using Supabase, make sure the latest SQL patch has been applied.</p>
...
# === HOTFIX: manager / TL lock exemption cleanup ===

def clear_privileged_lock_state(user):
    role = normalize_role((user or {}).get("role"))
    if role not in UNLOCK_EXEMPT_ROLES:
        return
    ...
    try:
        ...
        app.logger.warning(
            "Privileged presence unlock skipped for %s: %s",
            user.get("username"),
            exc,
        )

    try:
        for req in get_backend().list_rows("unlock_requests"):
            if req.get("user_id") == user.get("user_id") and (req.get("status") or "").strip().lower() == "pending":
                try:
                    get_backend().update_where(
                        "unlock_requests",
        ...
        app.logger.warning(
            "Privileged unlock request cleanup skipped for %s: %s",
            user.get("username"),
            exc,
        )

    try:
        invalidate_table_cache("presence", "unlock_requests")
    except Exception:
        pass
...
def reset_stale_presence_on_login(user):
    ...
    try:
        ...
        app.logger.warning(
            "Presence reset skipped during login for %s: %s",
            user.get("username"),
            exc,
        )

    if normalize_role(user.get("role")) in UNLOCK_EXEMPT_ROLES:
        clear_privileged_lock_state(user)
...
def human_lock_summary(user):
    ...
    presence = get_presence_for_user(user.get("user_id")) or {}
    req = pending_unlock_request_for(user.get("user_id"))
    return {
        "locked": to_boolish(presence.get("locked", "0")),
        "reason": lock_reason_message((req or {}).get("reason", "")),
        "request_id": (req or {}).get("request_id", ""),
        "requested_at": (req or {}).get("requested_at", ""),
        "pending": bool(req),
...
@app.before_request
def _manager_tl_lock_cleanup_hotfix():
    if not session.get("username"):
        return None
    user = current_user()
    if user and normalize_role(user.get("role")) in UNLOCK_EXEMPT_ROLES:
        clear_privileged_lock_state(user)
    return None


def attendance_breaks_v5():
    actor = current_user()
    if actor and normalize_role(actor.get("role")) in UNLOCK_EXEMPT_ROLES:
        clear_privileged_lock_state(actor)
    return attendance_breaks_v4()


app.view_functions["attendance_breaks"] = login_required(attendance_breaks_v5)


# === HOTFIX V2: broad role detection + hard bypass for manager/TL ===

_prev_normalize_role_hotfix_v2 = normalize_role

def normalize_role(role, designation=None):
    parts = []
    if role is not None:
    ...
    manager_signals = [
        "manager", "admin", "operations", "operation", "ops", "owner", "director", "management"
    ]
    tl_signals = [
        "team lead", "teamlead", "team leader", "teamleader", "tl", "lead"
    ]

    if any(sig in raw for sig in manager_signals) or any(sig in compact for sig in ["manager", "admin", "operations", "operation", "ops"]):
        return "manager"

    if (
        "team lead" in raw
        or "team leader" in raw
        or compact.startswith("tl")
        or compact.endswith("tl")
        or raw == "tl"
        or raw.startswith("tl ")
        or raw.endswith(" tl")
        or raw == "lead"
    ):
        return "tl"
...
def _unlock_exempt_user(user):
    if not user:
        return False
    return normalize_role(user.get("role"), user.get("designation")) in {"manager", "tl"}
...
def list_users():
    ...
    for row in rows:
        item = dict(row)
        item["role"] = normalize_role(item.get("role"), item.get("designation"))
        item["app_role"] = item["role"]
        item["theme_name"] = normalize_theme(item.get("theme_name"))
        item["is_active"] = "1" if to_boolish(item.get("is_active", "1")) else "0"
        normalized.append(item)
...
def clear_privileged_lock_state(user):
    if not _unlock_exempt_user(user):
        return
    try:
        ensure_presence_rows()
    except Exception:
    ...
    try:
        ...
        app.logger.warning("Privileged lock cleanup skipped for %s: %s", user.get("username"), exc)
    ...
    try:
        ...
        app.logger.warning("Privileged unlock request cleanup skipped for %s: %s", user.get("username"), exc)
...
def reset_stale_presence_on_login(user):
    ...
    try:
        ...
        app.logger.warning("Presence reset skipped during login for %s: %s", user.get("username"), exc)
    if _unlock_exempt_user(user):
        clear_privileged_lock_state(user)


def human_lock_summary(user):
    if not user:
        return {"locked": False, "reason": "", "request_id": "", "requested_at": "", "pending": False}
    if _unlock_exempt_user(user):
        clear_privileged_lock_state(user)
        return {"locked": False, "reason": "", "request_id": "", "requested_at": "", "pending": False}
    presence = get_presence_for_user(user.get("user_id")) or {}
...
def attendance_breaks_v6():
    actor = current_user()
    if actor and _unlock_exempt_user(actor):
        clear_privileged_lock_state(actor)
    return attendance_breaks_v4()


app.view_functions["attendance_breaks"] = login_required(attendance_breaks_v6)

try:
    funcs = app.before_request_funcs.get(None, [])
    app.before_request_funcs[None] = [f for f in funcs if getattr(f, "__name__", "") not in {"_manager_tl_lock_cleanup_hotfix", "final_security_scope_tick_v2"}]
except Exception:
    pass


@app.before_request
def _manager_tl_lock_cleanup_hotfix_v2():
    if not session.get("username"):
        return None
    user = current_user()
    if user and _unlock_exempt_user(user):
        clear_privileged_lock_state(user)
    return None


@app.before_request
def final_security_scope_tick_v3():
    if request.endpoint in {None, "static", "login", "health"}:
        return None
    if not session.get("username"):
        return None
...
# === v31 stripped speed build patch ===
REMOVED_SPEED_LABELS = {
    'Client Pipeline',
    'Revenue Hub',
    'AI Operations',
    'Meeting Room',
...
SIDEBAR_ITEMS = [item for item in SIDEBAR_ITEMS if item[0] not in REMOVED_SPEED_LABELS]
MODULE_SUMMARIES = {k: v for k, v in MODULE_SUMMARIES.items() if k not in REMOVED_SPEED_SLUGS}
GROWTH_MANAGER_LABELS = set()

_removed_endpoint_targets = {
    'client_pipeline_page',
    'revenue_hub_page',
    'testing_ai_page',
...
def visible_sidebar_items_v31(items, user):
    role = normalize_role((user or {}).get('role'))
    out = []
    for label, endpoint, kwargs in items:
        if label in REMOVED_SPEED_LABELS:
            continue
        if label == 'Admin Control' and role != 'manager':
            continue
        if label in {'Performance Centre', 'Recent Activity', 'Reports'} and role not in {'manager', 'tl'}:
            continue
        out.append((label, endpoint, kwargs))
    return out
...
def _removed_speed_redirect(*args, **kwargs):
    flash('This module was removed from the speed build to keep the CRM lighter and faster.', 'info')
    return redirect(url_for('dashboard'))


def _stripped_module_page(slug):
    if slug in REMOVED_SPEED_SLUGS:
        return _removed_speed_redirect()
    return module_page(slug)


def _stripped_module_page_v2(slug):
    if slug in REMOVED_SPEED_SLUGS:
        return _removed_speed_redirect()
    return module_page_v2(slug)


def _lightweight_dashboard():
    user = current_user()
    candidates = enrich_candidates()[:400]
    interviews = get_backend().list_rows('interviews')[:200]
    tasks = get_backend().list_rows('tasks')[:300]
    users = list_users()
    total_profiles = len([c for c in candidates if not to_boolish(c.get('is_duplicate', '0'))])
    today_calls = max(12, min(999, len(candidates) * 3))
    today_str = datetime.now().strftime('%Y-%m-%d')
    interviews_today = len([i for i in interviews if today_str in str(i.get('scheduled_at', ''))])
    active_managers = len([u for u in users if u.get('role') in {'manager', 'tl'}])
    due_tasks = []
    user_by_id = user_map('user_id')
    ...
    manager_monitoring = []
    for u in users:
        if u.get('role') not in {'recruiter', 'tl'}:
            continue
        ccount = len([c for c in candidates if (c.get('recruiter_code') or '') == (u.get('recruiter_code') or '')])
        open_tasks = len([t for t in tasks if t.get('assigned_to_user_id') == u.get('user_id') and (t.get('status') or '') != 'Closed'])
        manager_monitoring.append({'full_name': u.get('full_name'), 'designation': u.get('designation'), 'candidate_count': ccount, 'open_tasks': open_tasks})
    ...
    manager_monitoring = manager_monitoring[:6]
    unread_notes = user_notifications(user)[:5]
    pending_approvals = approval_requests_count_for(user)
    return render_template('dashboard.html', total_profiles=total_profiles, today_calls=today_calls, interviews_today=interviews_today, pending_approvals=pending_approvals, active_managers=active_managers, active_workers=len([u for u in users if normalize_role(u.get('role')) in {'recruiter','tl'}]), due_tasks=due_tasks, manager_monitoring=manager_monitoring, unread_notes=unread_notes)
...
def _lightweight_candidates():
    ...
    statuses = sorted({c.get('status', '') for c in enrich_candidates()[:500] if c.get('status')})
    return render_template('candidates.html', candidates=paged_rows, q=request.args.get('q',''), recruiters=recruiters_for_filters(), current_recruiter=recruiter, statuses=statuses, current_status=status, page=page, total_rows=total_rows, per_page=per_page)


_old_inject_growth_globals = inject_growth_globals

@app.context_processor
def inject_growth_globals_v31():
    base_ctx = _old_inject_growth_globals()
    user = current_user()
    base_ctx['sidebar_items'] = visible_sidebar_items_v31(SIDEBAR_ITEMS, user)
    base_ctx['leadership_role'] = normalize_role((user or {}).get('role')) in {'manager', 'tl'}
    base_ctx['manager_role'] = normalize_role((user or {}).get('role')) == 'manager'
    base_ctx['aaria_recent_history'] = []
    base_ctx['aaria_command_guide_pdf'] = ''
    return base_ctx

app.view_functions['dashboard'] = login_required(_lightweight_dashboard)
app.view_functions['candidates'] = login_required(_lightweight_candidates)
if 'module_page' in app.view_functions:
    app.view_functions['module_page'] = login_required(_stripped_module_page)
if 'module_page_v2' in app.view_functions:
    app.view_functions['module_page_v2'] = login_required(_stripped_module_page_v2)
for _endpoint in _removed_endpoint_targets:
    if _endpoint in app.view_functions:
        app.view_functions[_endpoint] = login_required(_removed_speed_redirect)


# === v32 ultra-light speed cleanup patch ===
RECENT_ACTIVITY_LABELS = {'Recent Activity'}
REMOVED_SPEED_LABELS = set(REMOVED_SPEED_LABELS) | RECENT_ACTIVITY_LABELS

def visible_sidebar_items_v32(items, user):
    role = normalize_role((user or {}).get('role'))
    out = []
    for label, endpoint, kwargs in items:
        if label in REMOVED_SPEED_LABELS:
        ...
        if label == 'Admin Control' and role != 'manager':
            continue
        if label in {'Performance Centre', 'Reports'} and role not in {'manager', 'tl'}:
            continue
        out.append((label, endpoint, kwargs))
    return out

def _recent_activity_removed():
    flash('Recent Activity was removed from this speed build.', 'info')
    return redirect(url_for('dashboard'))
...
def recruiter_performance_rows(selected_username=''):
    ...
    for user in users:
        ...
        user_tasks = [t for t in tasks if t.get('assigned_to_user_id') == user_id]
        user_activity = [a for a in activity if a.get('user_id') == user_id]
        calls = len([a for a in user_activity if a.get('action_type') in {'manual_call','aaria_call','dialer_call_ended'}])
        updates = len([a for a in user_activity if 'update' in (a.get('action_type') or '') or a.get('action_type') in {'candidate_created','bulk_note_added','task_created','dialer_note_saved'}])
        interviews = len([a for a in user_activity if 'interview' in (a.get('action_type') or '')])
        pres = presence.get(user_id, {})
        last_seen = pres.get('last_seen_at') or ''
        idle_minutes = 999
        ...
        rows.append({
            ...
            'collected': 0,
            'last_seen': display_ts(last_seen),
            'idle_state': f'No movement detected for the past {idle_threshold} minutes' if idle_minutes >= idle_threshold and idle_minutes < 999 else 'Active',
            'idle_alert': idle_minutes >= idle_threshold and idle_minutes < 999,
        })
    rows.sort(key=lambda x: (x['designation'] != 'Team Lead', -x['calls'], -x['candidate_count']))
    return rows

_old_inject_growth_globals_v31 = inject_growth_globals_v31
@app.context_processor
def inject_growth_globals_v32():
    base_ctx = _old_inject_growth_globals_v31()
    user = current_user()
    base_ctx['sidebar_items'] = visible_sidebar_items_v32(SIDEBAR_ITEMS, user)
    return base_ctx

if 'recent_activity_page' in app.view_functions:
    app.view_functions['recent_activity_page'] = login_required(_recent_activity_removed)
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=int(os.environ.get("PORT", 5000)), debug=False)


# === v33 hard speed build patch ===
HARD_SPEED_REMOVED_LABELS = {'Recent Activity','Meeting Room','Learning Hub','Social Career Crox','Wallet & Rewards','Payout Tracker','Client Pipeline','Revenue Hub','AI Operations'}

def visible_sidebar_items_v33(items, user):
    role = normalize_role((user or {}).get('role'))
    out = []
    for label, endpoint, kwargs in items:
        if label in HARD_SPEED_REMOVED_LABELS:
            continue
        if label == 'Admin Control' and role != 'manager':
            continue
        if label in {'Performance Centre', 'Reports'} and role not in {'manager', 'tl'}:
...
# Remove expensive per-request jobs from this speed build.
try:
    funcs = app.before_request_funcs.get(None, [])
    app.before_request_funcs[None] = [f for f in funcs if getattr(f, '__name__', '') not in {'aaria_before_request_tick', 'ensure_growth_suite_seeded_before_request'}]
except Exception:
    pass


# Tiny cached counters so every page does not re-count everything.
_SPEED_CACHE = {'notifications': {}, 'approvals': {}, 'dashboard': {}, 'performance': {}}

def _speed_cached(bucket, key, ttl, builder):
    store = _SPEED_CACHE.setdefault(bucket, {})
    now_ts = time.time()
    entry = store.get(key)
    if entry and now_ts - entry[0] < ttl:
        return entry[1]
    value = builder()
    store[key] = (now_ts, value)
    return value


def unread_notifications_count_fast(user):
    if not user:
        return 0
    uid = user.get('user_id') or user.get('username') or 'anon'
    return _speed_cached('notifications', uid, 30, lambda: len(user_notifications(user, unread_only=True)))


def approval_requests_count_fast(user):
    if not user:
        return 0
    uid = user.get('user_id') or user.get('username') or 'anon'
    return _speed_cached('approvals', uid, 30, lambda: approval_requests_count_for(user))


def _minimal_dashboard_v33():
    user = current_user() or {}
    role = normalize_role(user.get('role'))
    cache_key = f"{role}:{user.get('user_id','guest')}"
    payload = _speed_cached('dashboard', cache_key, 20, lambda: _build_minimal_dashboard_payload(user))
    return render_template('dashboard.html', **payload)


def _build_minimal_dashboard_payload(user):
    candidates = enrich_candidates()
    tasks = safe_list_rows('tasks')[:120]
    users = list_users()
    role = normalize_role((user or {}).get('role'))
    recruiter_code = (user or {}).get('recruiter_code','')
    if role == 'recruiter' and recruiter_code:
        candidates = [c for c in candidates if (c.get('recruiter_code') or '') == recruiter_code]
        tasks = [t for t in tasks if t.get('assigned_to_user_id') == user.get('user_id')]
    total_profiles = len([c for c in candidates if not to_boolish(c.get('is_duplicate', '0'))])
    open_tasks = [dict(t) for t in tasks if (t.get('status') or '').lower() not in {'closed','done'}][:5]
    due_tasks = []
    users_by_id = user_map('user_id')
    for task in open_tasks:
        assigned = users_by_id.get(task.get('assigned_to_user_id')) or {}
        task['full_name'] = assigned.get('full_name', task.get('assigned_to_name', ''))
        task['assigned_to'] = assigned.get('username', '')
        task['due_at'] = task.get('due_date', '')
        due_tasks.append(task)
    active_workers = len([u for u in users if normalize_role(u.get('role')) in {'recruiter','tl'}])
    return {
        'total_profiles': total_profiles,
        'today_calls': 0,
        'interviews_today': 0,
        'pending_approvals': approval_requests_count_fast(user),
        'active_managers': len([u for u in users if normalize_role(u.get('role')) in {'manager','tl'}]),
        'active_workers': active_workers,
        'due_tasks': due_tasks,
        'manager_monitoring': [],
...
def _hard_speed_candidates_v33():
    q = request.args.get('q', '').strip().lower()
    recruiter = request.args.get('recruiter', '').strip()
    status = request.args.get('status', '').strip()
    page = max(1, int(request.args.get('page', '1') or '1'))
    per_page = 40
    all_rows = [c for c in enrich_candidates() if not to_boolish(c.get('is_duplicate', '0'))]
    if q:
        all_rows = [c for c in all_rows if q in ' '.join([c.get('full_name',''), c.get('phone',''), c.get('location',''), c.get('status',''), c.get('jd_code',''), c.get('recruiter_code','')]).lower()]
    if recruiter:
        all_rows = [c for c in all_rows if c.get('recruiter_code') == recruiter]
    if status:
        all_rows = [c for c in all_rows if c.get('status') == status]
    all_rows.sort(key=lambda x: x.get('created_at', ''), reverse=True)
    total_rows = len(all_rows)
    start = (page - 1) * per_page
    rows = all_rows[start:start + per_page]
    statuses = sorted({c.get('status', '') for c in all_rows if c.get('status')})
    return render_template('candidates.html', candidates=rows, q=request.args.get('q',''), recruiters=recruiters_for_filters(), current_recruiter=recruiter, statuses=statuses, current_status=status, page=page, total_rows=total_rows, per_page=per_page)


def _performance_centre_fast_v33():
    user = current_user()
    selected_username = request.args.get('username','').strip()
    cache_key = selected_username or 'all'
    rows = _speed_cached('performance', cache_key, 30, lambda: recruiter_performance_rows(selected_username))
    people = [u for u in list_users() if normalize_role(u.get('role')) in {'recruiter','tl'}]
    people.sort(key=lambda x: (normalize_role(x.get('role')) != 'tl', x.get('full_name','')))
    total_calls = sum(int(r.get('calls',0) or 0) for r in rows)
    total_updates = sum(int(r.get('updates',0) or 0) for r in rows)
    total_interviews = sum(int(r.get('interviews',0) or 0) for r in rows)
    return render_template('performance_centre.html', rows=rows, alerts=[], people=people, selected_username=selected_username, total_calls=total_calls, total_updates=total_updates, total_interviews=total_interviews, total_billed=0, manager_role=normalize_role((user or {}).get('role')) == 'manager')


_old_growth_ctx_v32 = inject_growth_globals_v32
@app.context_processor
def inject_growth_globals_v33():
    base_ctx = _old_growth_ctx_v32()
    user = current_user()
    base_ctx['sidebar_items'] = visible_sidebar_items_v33(SIDEBAR_ITEMS, user)
    # keep links but stop hammering every request for counters.
    base_ctx['unread_notifications'] = unread_notifications_count_fast(user) if request.endpoint == 'notifications_page' else 0
    base_ctx['approval_requests_count'] = approval_requests_count_fast(user) if request.endpoint in {'approvals_page','dashboard'} else 0
    return base_ctx

app.view_functions['dashboard'] = login_required(_minimal_dashboard_v33)
app.view_functions['candidates'] = login_required(_hard_speed_candidates_v33)
if 'performance_centre_page' in app.view_functions:
    app.view_functions['performance_centre_page'] = login_required(leadership_required(_performance_centre_fast_v33))
