...
def login():
    if request.method == "POST":
        ...
        if user and user.get("password") == password and to_boolish(user.get("is_active", "1")):
            ...
            get_backend().delete_where("active_sessions", {"username": user["username"]})
            get_backend().insert("active_sessions", {"username": user["username"], "session_token": session_token, "ip_address": request.headers.get("X-Forwarded-For", request.remote_addr or ""), "user_agent": (request.headers.get("User-Agent", "")[:255]), "updated_at": now_iso()})
...
def enforce_single_active_session_v10():
    ...
    if current.get("session_token") != session_token:
        ...
        return redirect(url_for("login"))
    get_backend().update_where("active_sessions", {"username": username}, {"updated_at": now_iso(), "ip_address": request.headers.get("X-Forwarded-For", request.remote_addr or ""), "user_agent": (request.headers.get("User-Agent", "")[:255])})
    return None
...
def create_candidate():
    ...
    row = {
        ...
        "phone": request.form.get("phone", "").strip() or candidate.get("phone", ""),
