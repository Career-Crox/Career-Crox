...
def _lightweight_dashboard():
    ...
    return render_template('dashboard.html', total_profiles=total_profiles, today_calls=today_calls, interviews_today=interviews_today, pending_approvals=pending_approvals, active_managers=active_managers, active_workers=len([u for u in users if normalize_role(u.get('role')) in {'recruiter','tl'}]), recent_activity=recent_activity, due_tasks=due_tasks, manager_monitoring=manager_monitoring, unread_notes=unread_notes)
