...
# === speed + schema compatibility + english polish patch ===
import time
...
_FAST_TABLE_TTLS = {
    ...
    "presence": 10,
    "client_pipeline": 120,
    "client_requirements": 120,
    "revenue_entries": 120,
}
_FAST_NO_CACHE_TABLES = {"active_sessions", "unlock_requests", "aaria_queue", "activity_log", "scheduled_reports"}
_SUPABASE_MISSING_COLUMNS = {}
_GLOBAL_BACKEND_INSTANCE = None
_GLOBAL_BACKEND_FALLBACK_LOGGED = False


def _process_cache_key(backend, table_name):
    try:
        mode = (backend.describe() or {}).get("store_mode", "unknown")
    except Exception:
        mode = "unknown"
    return (mode, table_name)


def _clear_process_table_cache(*tables):
    if not tables:
        _FAST_TABLE_CACHE.clear()
        return
    table_names = set(tables)
    for key in list(_FAST_TABLE_CACHE.keys()):
        if key[1] in table_names:
            _FAST_TABLE_CACHE.pop(key, None)
...
def invalidate_table_cache(*tables):
    ...
    if cache:
        for table in tables:
            cache.pop(table, None)
    _clear_process_table_cache(*tables)
    for attr in ["_users_cache", "_user_map_cache", "_enriched_candidates_cache", "_candidate_map_cache", "_visible_candidates_cache", "_visible_candidate_ids_cache"]:
        if hasattr(g, attr):
            try:
                delattr(g, attr)
            except Exception:
...
def _extract_missing_column_name(error_text, table_name):
    text = str(error_text or "")
    patterns = [
        rf"column ['\"]?([A-Za-z_][A-Za-z0-9_]*)['\"]? of relation ['\"]?{re.escape(table_name)}['\"]? does not exist",
        rf"Could not find the ['\"]([A-Za-z_][A-Za-z0-9_]*)['\"] column of ['\"]?{re.escape(table_name)}['\"]?",
...
def _trim_known_missing(table_name, row):
    missing = _SUPABASE_MISSING_COLUMNS.get(table_name, set())
    if not missing:
        return dict(row)
    return {k: v for k, v in dict(row).items() if k not in missing}


def _retry_supabase_insert(raw_insert, table_name, row):
    current = _trim_known_missing(table_name, row)
    tried = set()
    last_exc = None
    while current:
        key = tuple(sorted(current.keys()))
        if key in tried:
            break
        tried.add(key)
        try:
            return raw_insert(table_name, current)
        except Exception as exc:
            last_exc = exc
            col = _extract_missing_column_name(exc, table_name)
            if not col or col not in current:
                raise
            _SUPABASE_MISSING_COLUMNS.setdefault(table_name, set()).add(col)
            current = {k: v for k, v in current.items() if k != col}
    if last_exc:
        raise last_exc
    return None


def _retry_supabase_bulk_insert(raw_bulk_insert, table_name, rows):
    payload = [_trim_known_missing(table_name, r) for r in rows]
    tried = set()
    last_exc = None
    while payload:
        keys = tuple(sorted(payload[0].keys())) if payload and payload[0] else tuple()
        if keys in tried:
            break
        tried.add(keys)
        try:
            return raw_bulk_insert(table_name, payload)
        except Exception as exc:
            last_exc = exc
            col = _extract_missing_column_name(exc, table_name)
            if not col:
...
def _retry_supabase_update(raw_update_where, table_name, filters, values):
    current = _trim_known_missing(table_name, values)
    tried = set()
    last_exc = None
    while current:
        key = tuple(sorted(current.keys()))
        ...
        tried.add(key)
        try:
            return raw_update_where(table_name, filters, current)
        except Exception as exc:
            last_exc = exc
            col = _extract_missing_column_name(exc, table_name)
            if not col or col not in current:
...
def _build_fast_backend(backend):
    if getattr(backend, "_fast_patch_wrapped", False):
        return backend
    ...
    store_mode = ""
    try:
        store_mode = (backend.describe() or {}).get("store_mode", "")
    except Exception:
        store_mode = ""

    def fast_list_rows(table_name, *args, **kwargs):
        if args or kwargs or table_name in _FAST_NO_CACHE_TABLES:
            return [dict(r) for r in raw_list_rows(table_name, *args, **kwargs)]
        key = _process_cache_key(backend, table_name)
        ttl = _FAST_TABLE_TTLS.get(table_name, 12)
        now_ts = time.time()
        cached = _FAST_TABLE_CACHE.get(key)
        if cached and cached[0] > now_ts:
            return [dict(r) for r in cached[1]]
        rows = [dict(r) for r in raw_list_rows(table_name)]
        _FAST_TABLE_CACHE[key] = (now_ts + ttl, [dict(r) for r in rows])
        return [dict(r) for r in rows]

    def fast_insert(table_name, row):
        invalidate_table_cache(table_name)
        if store_mode == "supabase":
            return _retry_supabase_insert(raw_insert, table_name, dict(row))
        return raw_insert(table_name, dict(row))

    def fast_bulk_insert(table_name, rows):
        invalidate_table_cache(table_name)
        if store_mode == "supabase":
            return _retry_supabase_bulk_insert(raw_bulk_insert, table_name, [dict(r) for r in rows])
        return raw_bulk_insert(table_name, [dict(r) for r in rows])

    def fast_update_where(table_name, filters, values):
        invalidate_table_cache(table_name)
        if store_mode == "supabase":
            return _retry_supabase_update(raw_update_where, table_name, dict(filters), dict(values))
        return raw_update_where(table_name, dict(filters), dict(values))

    def fast_delete_where(table_name, filters):
        invalidate_table_cache(table_name)
        return raw_delete_where(table_name, dict(filters))

    backend.list_rows = fast_list_rows
    if raw_insert:
        backend.insert = fast_insert
    if raw_bulk_insert:
        backend.bulk_insert = fast_bulk_insert
    if raw_update_where:
        backend.update_where = fast_update_where
    if raw_delete_where:
        backend.delete_where = fast_delete_where
    backend._fast_patch_wrapped = True
    return backend


def _make_global_backend():
    global _GLOBAL_BACKEND_INSTANCE, _GLOBAL_BACKEND_FALLBACK_LOGGED
    if _GLOBAL_BACKEND_INSTANCE is not None:
        return _GLOBAL_BACKEND_INSTANCE
    if USE_SUPABASE:
        try:
            sb = SupabaseBackend(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY)
            sb._request("GET", "users", params={"select": "user_id", "limit": 1})
            _GLOBAL_BACKEND_INSTANCE = _build_fast_backend(sb)
            return _GLOBAL_BACKEND_INSTANCE
        except Exception as exc:
            if not _GLOBAL_BACKEND_FALLBACK_LOGGED:
                app.logger.warning("Supabase not ready. Falling back to SQLite preview mode: %s", exc)
                _GLOBAL_BACKEND_FALLBACK_LOGGED = True
    _GLOBAL_BACKEND_INSTANCE = _build_fast_backend(SQLiteBackend(DB_PATH, SEED_FILE))
    return _GLOBAL_BACKEND_INSTANCE
...
def get_backend():
    backend = getattr(g, "backend", None)
    if backend is None:
        g.backend = _make_global_backend()
        backend = g.backend
    return backend


# Flash wrapper to keep visible system text professional English.
_original_flash_function = flash


def _professional_english_message(message):
    text = str(message or "")
    lowered = text.lower()
    exact = [
        ("Empty note save नहीं होगा. Software भी कुछ standards रखता है.", "An empty note cannot be saved."),
        ("Khali note save nahi hoga.", "An empty note cannot be saved."),
        ("Task create access sirf TL / Manager ke paas hai.", "Only team leads and managers can create tasks."),
        ("Manager / TL CRM par unlock approval apply nahi hota.", "CRM unlock approval does not apply to managers or team leads."),
        ("For job-seeking candidates, name, phone, qualification, and location are required.", "For job-seeking candidates, name, phone, qualification, and location are required."),
        ("Unlock request sent to the approval team.", "Unlock request sent to the approval team."),
    ...
    if any('\u0900' <= ch <= '\u097f' for ch in text):
        ...
        if "unlock" in lowered:
            return "Unlock request submitted successfully."
        if "note" in lowered:
            return "The note has been saved successfully."
        return "The action was completed."
    replacements = [
        (r"\bkhali\b", "empty"),
        (r"\bnahi\b", "not"),
        (r"\bhoga\b", "be allowed"),
        (r"\bkar diya\b", "completed"),
        (r"\bkar di\b", "completed"),
        (r"\bkhatam\b", "ended"),
        (r"\bka\b", "of"),
        (r"\bko\b", "for"),
    ]
...
def flash(message, category="message"):
    return _original_flash_function(_professional_english_message(message), category)


# Speed: avoid scanning every table on every request.
try:
    funcs = app.before_request_funcs.get(None, [])
    app.before_request_funcs[None] = [f for f in funcs if getattr(f, "__name__", "") not in {"final_security_scope_tick_v3", "_manager_tl_lock_cleanup_hotfix_v2"}]
except Exception:
    pass
...
def final_security_scope_tick_v4():
    if request.endpoint in {None, "static", "login", "health"}:
        return None
    if not session.get("username"):
        return None
    ...
    if _unlock_exempt_user(user):
        if now_ts - float(session.get("_privileged_cleanup_at", 0) or 0) > 45:
            clear_privileged_lock_state(user)
            session["_privileged_cleanup_at"] = now_ts
        return None

    if now_ts - float(session.get("_followup_check_at", 0) or 0) > 60:
        maybe_emit_due_followup_notifications(user)
        session["_followup_check_at"] = now_ts

    if now_ts - float(session.get("_lock_check_at", 0) or 0) > 20:
        ensure_presence_rows()
        maybe_auto_lock_overdue_break_v6(user)
        maybe_auto_lock_inactive_v6(user)
        maybe_auto_lock_no_call_v6(user)
        session["_lock_check_at"] = now_ts

    locked = get_presence_for_user(user.get("user_id")) or {}
    if to_boolish(locked.get("locked", "0")):
        allowed = {"attendance_breaks", "attendance_request_unlock", "attendance_unlock_decision", "attendance_ping", "notifications_page", "approvals_page", "logout", "stop_impersonation", "admin_page", "impersonate_login"}
        if request.endpoint not in allowed:
            flash("CRM access is currently locked. Please request unlock approval first.", "danger")
            return redirect(url_for("attendance_breaks"))
    return None
...
def lock_reason_message(reason):
    reason = (reason or "").strip()
    if "Late return from break" in reason:
        return "The recruiter CRM was auto-locked because the break time was exceeded. Unlock approval from a team lead or manager is required."
    if "No call dialed" in reason:
        return "The recruiter CRM was auto-locked because no call was dialed for 15 minutes. Unlock approval from a team lead or manager is required."
    if "Inactivity" in reason:
        return "The recruiter CRM was auto-locked after 5 minutes of inactivity. Unlock approval from a team lead or manager is required."
    return reason or "CRM access is locked. Unlock approval is required."


def _safe_approval_rows_for_attendance(actor):
    actor_role = normalize_role((actor or {}).get("role"))
    if actor_role in PRIVILEGED_ROLES:
        return pending_unlock_requests_for(actor)
    rows = [dict(r) for r in get_backend().list_rows("unlock_requests") if r.get("user_id") == actor.get("user_id")]
    rows.sort(key=lambda x: x.get("requested_at", ""), reverse=True)
    return rows


def attendance_breaks_v7():
    ensure_presence_rows()
    actor = current_user()
    actor_role = normalize_role((actor or {}).get("role"))
    if _unlock_exempt_user(actor):
        clear_privileged_lock_state(actor)
    users_by_id = user_map("user_id")
    rows = []
    for row in get_backend().list_rows("presence"):
    ...
    rows.sort(key=lambda x: (x.get("role", ""), x.get("full_name", "")))
    current_presence = get_presence_for_user(actor.get("user_id")) or {}
    unlock_requests = _safe_approval_rows_for_attendance(actor)
    return render_template(
        "attendance.html",
        presence_rows=rows,
        current_presence=current_presence,
...
app.view_functions["attendance_breaks"] = login_required(attendance_breaks_v7)


# Ensure add/update candidate writes do not crash on older Supabase schemas.
app.view_functions["update_candidate"] = login_required(update_candidate_v8)
app.view_functions["create_candidate"] = login_required(professional_create_candidate_v2)
app.view_functions["create_task"] = login_required(create_task_v3)
app.view_functions["reports_page"] = login_required(reports_page_v2)
