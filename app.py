...
def display_ts_seconds(value, default=""):
    if not value:
        return default
    text = str(value).replace("T", " ").strip()
    return text[:19]
...
class SupabaseBackend:
    ...
    def delete_where(self, table, filters):
        ...
        for k, v in (filters or {}).items():
            params[k] = f"eq.{v}"
        self._request("DELETE", table, params=params, json_body=None)
...
def add_note(candidate_code):
    ...
    if not body:
        flash("Empty notes cannot be saved.", "danger")
...
def professional_candidates():
    ...
    if q:
        rows = [c for c in rows if q in " ".join([c.get("candidate_id", c.get("code", "")), c.get("full_name", ""), c.get("phone", ""), c.get("location", ""), c.get("status", ""), c.get("process", ""), c.get("recruiter_code", ""), c.get("recruiter_name", ""), c.get("qualification", "")]).lower()]
    if recruiter:
        rows = [c for c in rows if c.get("recruiter_code") == recruiter]
    if status:
        rows = [c for c in rows if c.get("status") == status]
...
def aaria_parse_instruction(command_text, serial_hint=""):
    ...
    if action != "theme" and not candidate:
        return {"ok": False, "message": "Candidate not found. Try candidate ID, name, or phone digits again.", "action": action}
    msg = ""
    ...
    elif action == "interview_date": msg = f"Interview for {candidate.get('full_name')} is ready to be scheduled for {payload['scheduled_at']}."
...
def aaria_execute_instruction(command_text, serial_hint="", preview=False):
    ...
    if action == "theme":
        ...
        phone = normalize_phone(candidate.get("phone")); response["message"] = f"Dialer ready for {candidate.get('full_name')}."; response["action_link"] = f"tel:+91{phone}"; response["action_type"] = "call"; response["avatar_state"] = "call"; log_activity(user, "aaria_call", candidate.get("candidate_id"), {"phone": phone})
    elif action == "whatsapp":
        phone = normalize_phone(candidate.get("phone")); message = payload.get("message") or f"Hello {candidate.get('full_name')}, this message is from Career Crox."; response["message"] = f"WhatsApp draft ready for {candidate.get('full_name')}."; response["action_link"] = f"https://wa.me/91{phone}?text={quote(message)}"; response["action_type"] = "whatsapp"; response["avatar_state"] = "success"; log_activity(user, "aaria_whatsapp", candidate.get("candidate_id"), {"phone": phone})
...
def prune_activity_log(max_age_hours=24):
    cutoff = datetime.now() - timedelta(hours=max_age_hours)
    rows = sorted([dict(a) for a in get_backend().list_rows("activity_log")], key=lambda x: x.get("created_at", ""))
    for item in rows:
        raw = item.get("created_at") or ""
        dt = parse_dt_safe(raw)
        if dt and dt < cutoff:
            try:
                get_backend().delete_where("activity_log", {"activity_id": item.get("activity_id")})
            except Exception:
                pass


def recent_activity_payload(actor, selected_user_id=""):
    if normalize_role((actor or {}).get("role")) not in {"manager", "tl"}:
        return {"rows": [], "members": [], "summary": {}}
    users_by_id = user_map("user_id")
    visible_members = []
    for user in list_users():
        role = normalize_role(user.get("role"))
        if normalize_role(actor.get("role")) == "tl" and role != "recruiter":
            continue
        if normalize_role(actor.get("role")) == "manager" and role not in {"recruiter", "tl"}:
            continue
    ...
    allowed_ids = {m["user_id"] for m in visible_members}
    if selected_user_id and selected_user_id not in allowed_ids:
        selected_user_id = ""
    rows = []
    for item in sorted([dict(a) for a in get_backend().list_rows("activity_log")], key=lambda x: x.get("created_at", ""), reverse=True):
        ...
        if uid not in allowed_ids:
            continue
        if selected_user_id and uid != selected_user_id:
            continue
        meta = safe_json_loads(item.get("metadata"), {})
        user = users_by_id.get(uid) or {}
        rows.append({
            "when": display_ts_seconds(item.get("created_at")),
            "user_id": uid,
            "username": item.get("username", ""),
            "full_name": user.get("full_name", item.get("username", "")),
            "designation": user.get("designation", ""),
            "recruiter_code": user.get("recruiter_code", ""),
            "action": item.get("action_type", ""),
            "candidate_id": item.get("candidate_id", ""),
            "metadata": json.dumps(meta, ensure_ascii=False)[:180],
        })
        if len(rows) >= 120:
            break
    presence_map = {p.get("user_id"): dict(p) for p in get_backend().list_rows("presence")}
    summary_rows = []
    for member in visible_members:
        if selected_user_id and member["user_id"] != selected_user_id:
            continue
        latest_log = next((r for r in rows if r.get("user_id") == member["user_id"]), None)
        presence = presence_map.get(member["user_id"], {})
        last_seen_raw = presence.get("last_seen_at") or (latest_log or {}).get("when") or ""
        last_seen_dt = parse_dt_safe(last_seen_raw)
        diff_seconds = int((datetime.now() - last_seen_dt).total_seconds()) if last_seen_dt else 999999
        state = "Active" if diff_seconds < 180 else "No movement detected for the past 3 minutes"
        summary_rows.append({
            "user_id": member["user_id"],
            "full_name": member["full_name"],
            "designation": member["designation"],
            "recruiter_code": member["recruiter_code"],
            "last_seen": display_ts_seconds(last_seen_raw),
            "state": state,
            "state_class": "danger" if diff_seconds >= 180 else "success",
            "latest_action": (latest_log or {}).get("action", ""),
        })
    summary_rows.sort(key=lambda x: (x["state_class"] != "danger", x["full_name"]))
    visible_members.sort(key=lambda x: x["full_name"])
    return {"rows": rows, "members": visible_members, "summary": summary_rows}
...
def recent_activity_page():
    actor = current_user()
    if normalize_role(actor.get('role')) not in {'manager', 'tl'}:
        abort(403)
    selected_user_id = (request.args.get('user_id') or '').strip()
    payload = recent_activity_payload(actor, selected_user_id)
    return render_template('recent_activity.html', selected_user_id=selected_user_id, activity_rows=payload['rows'], activity_members=payload['members'], activity_summary=payload['summary'])


@app.route('/api/recent-activity-feed')
@login_required
def recent_activity_feed_api():
    actor = current_user()
    if normalize_role(actor.get('role')) not in {'manager', 'tl'}:
        return jsonify({'ok': False, 'message': 'Access denied.'}), 403
    selected_user_id = (request.args.get('user_id') or '').strip()
    payload = recent_activity_payload(actor, selected_user_id)
    return jsonify({'ok': True, 'rows': payload['rows'], 'summary': payload['summary']})
...
def aaria_parse_instruction_v2(command_text, serial_hint=""):
    ...
    if any(k in text for k in ["theme", "color theme", "colour theme"]):
        ...
        if candidate:
            payload["message"] = msg or f"Hello {candidate.get('full_name','')}, this message is from Career Crox."
        else:
    ...
    if action in {"call", "whatsapp", "candidate_status", "interview_date", "add_note", "phone_update"} and not candidate:
        return {"ok": False, "message": "Candidate not found. Try name, candidate ID, serial, or phone digits.", "action": action}
    ...
    if action == "candidate_status":
        ...
        msg = f"Interview for {candidate.get('full_name')} is ready to be scheduled for {payload['scheduled_at']}."
    elif action == "add_note":
...
def lock_reason_message(reason):
    ...
    if "Late return from break" in reason:
        return "The CRM auto-locked 90 seconds after the break ended. Team lead or manager approval is required to unlock it."
...
def create_unlock_request_if_missing(user, reason):
    ...
    notify_users([u.get("user_id") for u in approver_users_for(user)], "Unlock request", f"{user.get('full_name')} needs unlock approval.", "attendance", {"user_id": user.get("user_id"), "reason": reason})
    return row
...
def split_aaria_commands_v3(command_text):
    ...
    items = [re.sub(r'^\d+[\.)]\s*', '', p).strip(' -') for p in re.split(r'\s*;\s*', raw) if p.strip(' -')]
    return items[:25]
