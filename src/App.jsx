...
function App() {
  ...
  useEffect(() => {
    setVisitedTabs((current) => (current.includes(active) ? current : [...current, active]));
  }, [active]);
    ...
    <div className="app-shell">
      ...
      <main className="main-area">
        ...
        {visitedTabs.includes('dashboard') && (
          <section className={active === 'dashboard' ? 'view-panel active' : 'view-panel hidden'}>
            <DashboardView api={api} refreshKey={refreshKey} />
          </section>
        )}
        {visitedTabs.includes('candidates') && (
          <section className={active === 'candidates' ? 'view-panel active' : 'view-panel hidden'}>
            <CandidatesView api={api} user={user} setNotice={setNotice} bump={() => setRefreshKey((n) => n + 1)} />
          </section>
        )}
        {visitedTabs.includes('tasks') && (
          <section className={active === 'tasks' ? 'view-panel active' : 'view-panel hidden'}>
            <TasksView api={api} user={user} setNotice={setNotice} />
          </section>
        )}
        {visitedTabs.includes('jds') && (
          <section className={active === 'jds' ? 'view-panel active' : 'view-panel hidden'}>
            <JdsView api={api} user={user} setNotice={setNotice} />
          </section>
        )}
        {visitedTabs.includes('interviews') && (
          <section className={active === 'interviews' ? 'view-panel active' : 'view-panel hidden'}>
            <InterviewsView api={api} user={user} setNotice={setNotice} />
          </section>
        )}
        {visitedTabs.includes('submissions') && (
          <section className={active === 'submissions' ? 'view-panel active' : 'view-panel hidden'}>
            <SubmissionsView api={api} user={user} setNotice={setNotice} />
          </section>
        )}
        {visitedTabs.includes('users') && (
          <section className={active === 'users' ? 'view-panel active' : 'view-panel hidden'}>
            <UsersView api={api} user={user} setNotice={setNotice} />
          </section>
        )}
        {visitedTabs.includes('notifications') && (
          <section className={active === 'notifications' ? 'view-panel active' : 'view-panel hidden'}>
            <NotificationsView api={api} user={user} setNotice={setNotice} />
          </section>
        )}
        {visitedTabs.includes('activity') && (
          <section className={active === 'activity' ? 'view-panel active' : 'view-panel hidden'}>
            <ActivityView api={api} />
          </section>
        )}
        {visitedTabs.includes('settings') && (
          <section className={active === 'settings' ? 'view-panel active' : 'view-panel hidden'}>
            <SettingsView api={api} user={user} setNotice={setNotice} />
          </section>
        )}
      </main>
    </div>
