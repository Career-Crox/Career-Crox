...
function CandidatesView({ api, user, setNotice, bump }) {
  ...
  return (
    <div className="stack">
      ...
      <div className="panel glass-card">
        <PanelHeader title="Candidates" subtitle="Fast list, notes, import, export, and approval tracking." />
        <div className="toolbar-row wrap-toolbar">
          ...
          <button className="secondary-btn" onClick={() => downloadFile('/api/candidates/template', 'career-crox-candidate-template.xlsx')}>Template</button>
          <button className="secondary-btn" onClick={() => downloadFile('/api/candidates/export', 'career-crox-candidates.xlsx')}>Export</button>
        ...
        {loading ? <PanelLoading label="Loading candidates" /> : (
          <DataTable columns={[
            ['candidate_id', 'ID'], ['full_name', 'Full name'], ['phone', 'Phone'], ['process', 'Process'], ['status', 'Status'], ['follow_up_at', 'Follow-up'], ['approval_status', 'Approval'], ['recruiter_name', 'Recruiter']
          ]} rows={rows} renderActions={(row) => (
            <div className="row-actions">
              <button onClick={() => setEditor(row)}>Edit</button>
              <button onClick={() => openNotes(row)}>Notes</button>
              {user.role !== 'recruiter' ? <button className="danger-link" onClick={() => removeCandidate(row)}>Delete</button> : null}
            </div>
          )} />
        )}
      </div>
